...
export const KNOWN_CURRENCY_CODES: ReadonlySet<string> = new Set(LIST_CURRENCIES.map(c => c.code));

const currencyByCode = new Map(LIST_CURRENCIES.map(c => [c.code, c]));

export function getKnownCurrency(code: string): (typeof LIST_CURRENCIES)[number] | undefined {
    return currencyByCode.get(code.toUpperCase());
}

export function isZeroDecimal(code: string): boolean {
    return ZERO_DECIMAL_CODES.has(code.toUpperCase());
}

export function getDefaultPrecision(code: string): number {
