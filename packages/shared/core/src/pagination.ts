...
type FilterOutputValue<TFilter extends ListFilterContract> = TFilter extends {
  kind: "number";
}
  ? number
...
type FilterSchema<TFilter extends ListFilterContract> = TFilter extends {
  cardinality: "multi";
}
...
function createFilterSchema<TFilter extends ListFilterContract>(
  ...
  if (filter.cardinality === "multi") {
    ...
    }, z.array(valueSchema).optional()) as unknown as FilterSchema<TFilter>;
  }
...
>(contract: ListQueryContract<TSortableColumns, TFilters>) {
  ...
  for (const key of Object.keys(contract.filters) as (keyof TFilters)[]) {
    ...
    filtersShape[key] = createFilterSchema(
      filter,
    ) as FiltersShape<TFilters>[typeof key];
  }
