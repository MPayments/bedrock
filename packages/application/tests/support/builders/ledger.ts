...
import type { TbClient } from "../../../src/ledger/tb";
import type { CreateIntentLine, OperationIntent } from "../../../src/ledger/types";
          ...
          returning: vi.fn(() => {
            ...
            if (insertedValues[0]?.tbAccountId !== undefined) {
              return Promise.resolve(
                insertedValues.map((value: any, index: number) => ({
                  id: value.id ?? `book-account-${index + 1}`,
                  tbLedger: value.tbLedger,
                  tbAccountId: value.tbAccountId,
                })),
              );
            }
            ...
            return Promise.resolve(
              insertedValues.map((_: any, index: number) => ({
                id: `id-${index + 1}`,
              })),
            );
          }),
