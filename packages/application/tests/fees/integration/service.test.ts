  ...
  return {
    async findByCode(code: string) {
      ...
      const rows = await db
        ...
        .where(eq(schema.currencies.code, normalized))
        .limit(1);

      if (!rows[0]) {
        throw new Error(`Unknown currency code: ${normalized}`);
      }
...
describe("Fees Service Integration Tests", () => {
  const currenciesService = createCurrencyLookup();
