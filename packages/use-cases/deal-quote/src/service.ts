import {
  DealCalculationQuoteInactiveError,
  DealCalculationQuoteNotAcceptedError,
  DealCalculationSourceNotFoundError,
...
interface AcceptedQuoteWorkflow {
  acceptedQuote: {
...
interface DealQuoteServiceDealsPort {
  deals: {
    commands: {
      appendTimelineEvent(input: any): Promise<unknown>;
      linkCalculationFromAcceptedQuote(input: any): Promise<unknown>;
    };
    queries: {
      findWorkflowById(dealId: string): Promise<AcceptedQuoteWorkflow | null>;
    };
  };
}

interface DealQuoteServiceCalculationsPort {
  calculations: {
    commands: {
...
type CanonicalCalculation = Awaited<
  ReturnType<DealQuoteServiceCalculationsPort["calculations"]["commands"]["create"]>
>;
...
export interface CreateDealQuoteServiceInput {
  calculations: DealQuoteServiceCalculationsPort;
  currencies: Pick<CurrenciesService, "findByCode" | "findById">;
  deals: DealQuoteServiceDealsPort;
...
async function requireCurrentAcceptedQuote(input: {
  ...
  dealId: string;
  deals: CreateDealQuoteServiceInput["deals"];
  now: Date;
  quoteId: string;
}) {
  ...
  if (!workflow) {
    throw new DealCalculationSourceNotFoundError(input.dealId);
  }

  if (workflow.acceptedQuote?.quoteId !== input.quoteId) {
    throw new DealCalculationQuoteNotAcceptedError(input.dealId, input.quoteId);
  }
  ...
  if (workflow.acceptedQuote.quoteStatus !== "active" && !sameUsedDocument) {
    throw new DealCalculationQuoteInactiveError(
      input.quoteId,
  ...
  if (
    ...
    throw new DealCalculationQuoteInactiveError(input.quoteId, "expired");
  }
...
export function createDealQuoteService(
  deps: CreateDealQuoteServiceInput,
) {
  return {
    async createCalculationFromAcceptedQuote(input: {
      ...
      if (quote.status !== "active") {
        throw new DealCalculationQuoteInactiveError(quote.id, quote.status);
      }

      if (quote.expiresAt.getTime() <= Date.now()) {
        throw new DealCalculationQuoteInactiveError(quote.id, "expired");
      }
...
export type DealQuoteService = ReturnType<typeof createDealQuoteService>;
