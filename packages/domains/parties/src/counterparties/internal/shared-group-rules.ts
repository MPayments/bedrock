...
export async function ensureCustomerGroupForCustomer(input: {
  ...
  const customer = (customerResult.rows?.[0] as {
    ...
    ? {
        displayName:
          ((customerResult.rows?.[0] as { display_name?: string | null })
            .display_name ?? null),
      }
    : null;
  ...
  const resolvedDisplayName = customer.displayName ?? `Customer ${customerId}`;
  const groupName = input.buildGroupName
    ? input.buildGroupName(resolvedDisplayName, customerId)
    : resolvedDisplayName;
