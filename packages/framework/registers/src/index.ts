...
export type RegisterRecord = Record<string, string | null | undefined>;
...
export interface DimensionRegistry {
  ...
  resolveLabelsFromRecords: (input: {
    db: Database;
    records: readonly (RegisterRecord | null | undefined)[];
  }) => Promise<Record<string, Record<string, string>>>;
}
...
export function createDimensionRegistry(
  entries: readonly DimensionRegistryEntry[],
  options?: {
    ttlMs?: number;
  ...
  async function resolveLabelsFromRecords(input: {
    db: Database;
    records: readonly (RegisterRecord | null | undefined)[];
  }) {
