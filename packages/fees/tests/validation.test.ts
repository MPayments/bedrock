...
describe("fees validation", () => {
    it("normalizes fee components and enforces paired ledger account keys", () => {
    ...
    it("validates fee-rule shape constraints and normalizes fixed currency", () => {
        expect(() => validateUpsertFeeRuleInput({
    ...
    it("rejects invalid inputs for quote-fee and quote-component APIs", () => {
        expect(() => validateFxQuoteFeeCalculation({
            fromCurrency: "USD",
            toCurrency: "EUR",
            principalMinor: 0n,
            at: new Date(),
        ...
        expect(() => validateSaveQuoteFeeComponentsInput({
            quoteId: "not-a-uuid",
            components: [],
        })).toThrowError(/saveQuoteFeeComponents:/);

        expect(() => validateGetQuoteFeeComponentsInput({
            quoteId: "not-a-uuid",
    ...
    it("validates rule-resolution input and formats contextual validation errors", () => {
        ...
        expect(() => validateInput(z.string().min(3), "x", "customContext")).toThrowError(
            /^customContext:/
        );
    });
    ...
    it("keeps error hierarchy and public API exports stable", () => {
        ...
        const validPayload = feesApi.getQuoteFeeComponentsSchema.parse({ quoteId: VALID_QUOTE_ID });
