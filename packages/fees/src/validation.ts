...
export const upsertFeeRuleSchema = z
    .object({
        ...
        bps: nonNegativeIntegerSchema.max(10000, "bps cannot exceed 10000 (100%)").optional(),
    ...
    .refine(
        ...
        {
            message: "calcMethod=bps requires bps and forbids fixedAmountMinor; calcMethod=fixed requires fixedAmountMinor",
            path: ["calcMethod"],
        }
    )
    .refine(
...
export function validateSaveQuoteFeeComponentsInput(input: unknown) {
    return validateInput(saveQuoteFeeComponentsSchema, input, "saveQuoteFeeComponents");
}

export function validateGetQuoteFeeComponentsInput(input: unknown) {
    return validateInput(getQuoteFeeComponentsSchema, input, "getQuoteFeeComponents");
}
