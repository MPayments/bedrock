...
export function createPayoutHandlers(context: TreasuryServiceContext) {
    ...
    async function initiatePayout(input: InitiatePayoutInput) {
        ...
        log.debug("initiatePayout start", { orderId: validated.orderId, railRef: validated.railRef });
        ...
        return db.transaction(async (tx: Transaction) => {
            ...
            const moved = await tx
                ...
                .where(and(eq(schema.paymentOrders.id, validated.orderId), eq(schema.paymentOrders.status, TreasuryOrderStatus.FX_EXECUTED)))
                .returning({ id: schema.paymentOrders.id });

            if (moved.length) {
                log.info("initiatePayout ok", { orderId: validated.orderId, entryId, pendingTransferId });
            ...
            if (isSameEntryInAllowedState(
                ...
                log.debug("initiatePayout idempotent", { orderId: validated.orderId, status: st });
    ...
    async function settlePayout(input: SettlePayoutInput) {
        ...
        log.debug("settlePayout start", { orderId: validated.orderId, railRef: validated.railRef });
        ...
        return db.transaction(async (tx: Transaction) => {
            ...
            const moved = await tx
                ...
                .where(and(eq(schema.paymentOrders.id, validated.orderId), eq(schema.paymentOrders.status, TreasuryOrderStatus.PAYOUT_INITIATED)))
                .returning({ id: schema.paymentOrders.id });
            ...
            if (isSameEntryInAllowedState(
                ...
                log.debug("settlePayout idempotent", { orderId: validated.orderId, status: st });
                return entryId;
            }
    ...
    async function voidPayout(input: VoidPayoutInput) {
        ...
        log.debug("voidPayout start", { orderId: validated.orderId, railRef: validated.railRef });
        ...
        return db.transaction(async (tx: Transaction) => {
            ...
            if (isSameEntryInAllowedState(
                ...
                log.debug("voidPayout idempotent", { orderId: validated.orderId, status: st });
                return entryId;
            }
