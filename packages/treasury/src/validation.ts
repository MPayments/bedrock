...
const feeComponentInputSchema = z
  ...
  .transform((data) => ({
    ...
    id:
      data.id ??
      `manual:${data.kind}:${data.currency}:${data.amountMinor.toString()}`,
    source: "manual" as const,
    settlementMode: data.settlementMode ?? "in_ledger",
    accountingTreatment:
      data.accountingTreatment ??
      (data.settlementMode === "separate_payment_order"
        ? "pass_through"
        : "income"),
  }));
...
const adjustmentInputSchema = z
  ...
  .transform((data) => ({
    ...
    id:
      data.id ??
      `adjustment:${data.kind}:${data.effect}:${data.currency}:${data.amountMinor.toString()}`,
    source: "manual" as const,
    settlementMode: data.settlementMode ?? "in_ledger",
  }));
...
export type ExecuteFxAdjustmentComponent = z.output<
...
export const initiateFeePaymentInputSchema = z.object({
  ...
  payoutCounterpartyId: uuidSchema,
  payoutOperationalAccountId: uuidSchema,
  railRef: railRefSchema,
  timeoutSeconds: z.number().int().positive().optional(),
  occurredAt: z.date(),
});

export type InitiateFeePaymentInput = z.infer<
  typeof initiateFeePaymentInputSchema
>;
...
export function validateFundingSettledInput(
  input: unknown,
): FundingSettledInput {
...
export function validateExecuteFxInput(
  input: unknown,
): ExecuteFxValidatedInput {
  return validateInput(executeFxInputSchema, input, "executeFx");
}

export function validateInitiatePayoutInput(
  input: unknown,
): InitiatePayoutInput {
...
export function validateInitiateFeePaymentInput(
  input: unknown,
): InitiateFeePaymentInput {
  return validateInput(
    initiateFeePaymentInputSchema,
    input,
    "initiateFeePayment",
  );
}

export function validateSettleFeePaymentInput(
  input: unknown,
): SettleFeePaymentInput {
  return validateInput(settleFeePaymentInputSchema, input, "settleFeePayment");
}

export function validateVoidFeePaymentInput(
  input: unknown,
): VoidFeePaymentInput {
