...
} from "@bedrock/finance/reconciliation/schema";
...
async function findDocumentById(
  documents: ReconciliationServiceDeps["documents"],
  ...
  if (!documents?.hasDocument) {
    return documentId;
  }

  const exists = await documents.hasDocument(documentId);
...
async function getExternalRecordsForRun(
...
): Promise<ReconciliationExternalRecord[]> {
  const filters = [
  ...
  return (await db
    ...
    .orderBy(
      ...
      schema.reconciliationExternalRecords.id,
    )) as ReconciliationExternalRecord[];
}
  ...
  async function ingestExternalRecord(input: {
    ...
    return db.transaction(async (tx) =>
      idempotency.withIdempotencyTx({
        ...
        serializeResult: (result) => ({ id: result.id }),
        loadReplayResult: async () => {
          const replayRows = (await tx
            ...
            .limit(1)) as ReconciliationExternalRecord[];

          const record = replayRows[0];
        ...
        handler: async () => {
          ...
          const insertedRows = (await tx
            ...
            .returning()) as ReconciliationExternalRecord[];
      ...
      idempotency.withIdempotencyTx({
        ...
        loadReplayResult: async ({ storedResult }) => {
          const replayRows = (await tx
            ...
            .limit(1)) as ReconciliationRun[];

          const run = replayRows[0];
        ...
        handler: async () => {
          ...
          for (const record of records) {
            resolutions.push({
              record,
              resolution: await resolveMatch(tx, documents, record),
            });
          }
          ...
          const runRows = (await tx
            ...
            .returning()) as ReconciliationRun[];
