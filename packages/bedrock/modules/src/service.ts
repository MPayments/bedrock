...
function normalizeManifestsForChecksum(manifests: readonly ModuleManifest[]) {
  return [...manifests]
    .map((manifest) => ({
      ...manifest,
      capabilities: {
...
function validateModuleManifests(manifests: readonly ModuleManifest[]) {
  const issues: string[] = [];
