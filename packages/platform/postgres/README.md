# @bedrock/platform-postgres

Drizzle-based database package for the financial core.

## What it provides

- Shared `db` client (`packages/platform/postgres/src/client.ts`)
- Shared `Database` type
- Aggregated schema registry (`packages/platform/postgres/src/schema/index.ts`)
- Centralized migrations (`packages/platform/postgres/migrations`)
...
- `packages/platform/*/src/schema.ts` when platform packages own schema
