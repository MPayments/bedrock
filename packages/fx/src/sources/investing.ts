import { ModuleClient, SessionClient, type TlsClientResponse } from "tlsclientwrapper";

import { RateSourceSyncError } from "../errors";
import { type FxRateSourceFetchResult, type FxRateSourceProvider } from "./types";
import { parseDecimalToFraction } from "../internal/fraction";
...
export function createInvestingRateSourceProvider(deps: InvestingRateSourceProviderDeps = {}): FxRateSourceProvider {
        ...
        try {
            for (const mapping of pairMappings) {
                try {
                    ...
                    if (latestPublishedAt === undefined || pairRate.publishedAt.getTime() > latestPublishedAt.getTime()) {
            ...
            if (latestPublishedAt === undefined) {
                throw new RateSourceSyncError("investing", "provider returned no parseable rates", lastError);
            }
...
async function fetchPairRateWithRetry(input: {
    ...
    throw new RateSourceSyncError("investing", `all retries failed for pair ${input.mapping.base}/${input.mapping.quote}`, lastError);
}
...
async function fetchPairRate(input: {
    ...
    try {
        ...
        throw new RateSourceSyncError("investing", `request failed for pair ${mapping.base}/${mapping.quote}`, error);
    }

    if (response.status < 200 || response.status >= 300) {
        throw new RateSourceSyncError("investing", `HTTP ${response.status} for pair ${mapping.base}/${mapping.quote}`);
    }

    const payload = await parsePayload(response, `${mapping.base}/${mapping.quote}`);
    const candles = extractCandles(payload, `${mapping.base}/${mapping.quote}`);
    const latestCandle = candles.at(-1)!;

    const publishedAt = extractTimestamp(latestCandle, `${mapping.base}/${mapping.quote}`);
    const close = extractClosePrice(latestCandle, `${mapping.base}/${mapping.quote}`);
...
async function parsePayload(response: InvestingResponse, pairKey: string): Promise<{ data?: unknown[] }> {
    if (response.json && isRecord(response.json) && Array.isArray(response.json.data)) {
        return response.json as { data?: unknown[] };
    }

    const rawText = await response.getText();
    if (!rawText || !rawText.trim()) {
        throw new RateSourceSyncError("investing", `empty response body for pair ${pairKey}`);
    }

    try {
        return JSON.parse(rawText) as { data?: unknown[] };
    } catch (error) {
        throw new RateSourceSyncError("investing", `invalid JSON payload for pair ${pairKey}`, error);
    }
}

function extractCandles(payload: { data?: unknown[] }, pairKey: string) {
    if (!Array.isArray(payload.data) || payload.data.length === 0) {
        throw new RateSourceSyncError("investing", `empty chart data for pair ${pairKey}`);
    }
    return payload.data;
}
...
function extractTimestamp(candle: unknown, pairKey: string) {
    if (!Array.isArray(candle) || candle.length < 1) {
        throw new RateSourceSyncError("investing", `invalid candle shape for pair ${pairKey}`);
    }

    const value = Number(candle[0]);
    if (!Number.isFinite(value) || value <= 0) {
        throw new RateSourceSyncError("investing", `invalid candle timestamp for pair ${pairKey}`);
    }

    const date = new Date(value);
    if (Number.isNaN(date.getTime())) {
        throw new RateSourceSyncError("investing", `invalid publishedAt for pair ${pairKey}`);
    }
    return date;
}
...
function extractClosePrice(candle: unknown, pairKey: string) {
    if (!Array.isArray(candle) || candle.length < 5) {
        throw new RateSourceSyncError("investing", `invalid candle close for pair ${pairKey}`);
    }

    const close = candle[4];
    if (typeof close === "number") {
        if (!Number.isFinite(close) || close <= 0) {
            throw new RateSourceSyncError("investing", `invalid close value for pair ${pairKey}`);
        }
        return String(close);
    }
    ...
    throw new RateSourceSyncError("investing", `unsupported close type for pair ${pairKey}`);
}
