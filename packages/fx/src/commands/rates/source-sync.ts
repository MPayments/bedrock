...
import { type FxRateSource, type FxRateSourceStatus, type FxRateSourceSyncResult } from "../../sources/types";
...
export function createRateSourceHandlers(context: FxServiceContext) {
    ...
    function sourcePairKey(source: FxRateSource, baseCurrencyId: string, quoteCurrencyId: string) {
    ...
    function toStatus(row: FxRateSourceRow, now: Date): FxRateSourceStatus {
        const expiresAt = row.lastSyncedAt
            ? new Date(row.lastSyncedAt.getTime() + row.ttlSeconds * 1000)
            : null;
    ...
    async function getSourceRow(source: FxRateSource): Promise<FxRateSourceRow | null> {
    ...
    async function getOrCreateSourceRow(source: FxRateSource): Promise<FxRateSourceRow> {
        ...
        if (!raced) {
            throw new RateSourceSyncError(source, "cannot initialize source status row");
        }

        return raced;
    }

    async function getRateSourceStatuses(now = new Date()): Promise<FxRateSourceStatus[]> {
        const sources = configuredSources();
    ...
    async function getSourceRateRows(source: FxRateSource, baseCurrencyId: string, quoteCurrencyId: string) {
    ...
    async function getManualRateRows(baseCurrencyId: string, quoteCurrencyId: string) {
    ...
    async function persistRates(source: FxRateSource, rates: {
        base: string;
        quote: string;
        rateNum: bigint;
        rateDen: bigint;
        ...
        for (const rate of rates) {
            dedupedMap.set(`${rate.base}|${rate.quote}|${rate.asOf.toISOString()}`, rate);
        }
        const dedupedRates = [...dedupedMap.values()];

        const currencyCodes = [...new Set(dedupedRates.flatMap((rate) => [rate.base, rate.quote]))];
        ...
        for (const code of currencyCodes) {
            try {
                ...
                log.warn("Skip FX rate for unknown currency", { source, currency: code });
            }
        }

        const validRows = dedupedRates.filter((rate) => currencyIdByCode.has(rate.base) && currencyIdByCode.has(rate.quote));
        if (!validRows.length) {
            throw new RateSourceSyncError(source, "provider returned no rates for known currencies");
        }
    ...
    async function markSyncSuccess(source: FxRateSource, now: Date, publishedAt: Date) {
        await db
            .insert(schema.fxRateSources)
            .values({
                source,
    ...
    async function markSyncFailure(source: FxRateSource, now: Date, error: unknown) {
        ...
        const previous = sourceStatusBySource.get(source) ?? buildDefaultSourceRow(source);
        setCachedSourceRow({
            ...previous,
            source,
            lastStatus: "error",
    ...
    async function performSync(source: FxRateSource, force: boolean, now: Date): Promise<FxRateSourceSyncResult> {
    ...
    async function syncRatesFromSource(input: { source: FxRateSource; force?: boolean; now?: Date }): Promise<FxRateSourceSyncResult> {
        ...
        const promise = performSync(validated.source, validated.force ?? false, now);
    ...
    async function ensureSourceFresh(source: FxRateSource, now = new Date()): Promise<FxRateSourceStatus> {
        ...
        try {
            const syncResult = await syncRatesFromSource({ source, force: true, now });
    ...
    async function getLatestRateBySource(baseCurrencyId: string, quoteCurrencyId: string, asOf: Date, source: FxRateSource) {
        const rows = await getSourceRateRows(source, baseCurrencyId, quoteCurrencyId);
        return findLatestRate(rows, asOf);
    }

    async function getLatestManualRate(baseCurrencyId: string, quoteCurrencyId: string, asOf: Date) {
