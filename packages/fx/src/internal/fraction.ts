...
export function parseDecimalToFraction(
    ...
    const normalized = normalizeNumberString(input).replace(",", ".").toUpperCase();
    ...
    if (eIndex !== -1) {
        ...
        if (eIndex === 0 || eIndex === normalized.length - 1 || normalized.indexOf("E", eIndex + 1) !== -1) {
            throw new Error(`invalid decimal number: ${input}`);
        }
    ...
    if (!intPart.length || !isDigits(intPart) || (fracPart.length > 0 && !isDigits(fracPart))) {
        throw new Error(`invalid decimal number: ${input}`);
    }
