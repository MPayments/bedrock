...
export const CounterpartyAccountProviderSchema = z.object({
  id: z.uuid(),
  ...
  if (data.type === "bank") {
    if (data.country === "RU") {
      ...
      if (data.bic) {
        ctx.addIssue({
          code: "custom",
          path: ["bic"],
          message: "BIC must not be set for non-Russian banks",
        });
      }
    }
  } else {
    if (data.bic) {
      ctx.addIssue({
        code: "custom",
        path: ["bic"],
        message: "BIC is only applicable for banks",
      });
    }
    if (data.swift) {
      ctx.addIssue({
        code: "custom",
        path: ["swift"],
        message: "SWIFT is only applicable for banks",
      });
    }
  }
}

export const CreateProviderInputSchema = providerFieldsSchema.superRefine(
  refineProviderBicSwift,
);
...
const PROVIDERS_SORTABLE_COLUMNS = [
  "name",
  "type",
  "country",
  "createdAt",
...
interface ProvidersListFilters {
  type: {
    kind: "string";
    cardinality: "multi";
    enumValues: readonly ["bank", "exchange", "blockchain", "custodian"];
  };
  country: { kind: "string"; cardinality: "multi" };
  name: { kind: "string"; cardinality: "single" };
}

export const COUNTERPARTY_ACCOUNT_PROVIDERS_LIST_CONTRACT: ListQueryContract<
...
export const COUNTERPARTY_ACCOUNTS_LIST_CONTRACT: ListQueryContract<
...
export const ResolveCounterpartyAccountBindingsInputSchema = z.object({
    accountIds: z.array(z.uuid()).min(1).max(100),
});
export type ResolveCounterpartyAccountBindingsInput = z.infer<typeof ResolveCounterpartyAccountBindingsInputSchema>;

export const CounterpartyAccountBindingSchema = z.object({
    accountId: z.uuid(),
    bookId: z.uuid(),
    counterpartyId: z.uuid(),
    currencyId: z.uuid(),
...
export type CounterpartyAccountBinding = z.infer<typeof CounterpartyAccountBindingSchema>;
