...
export async function resolveGroupMembershipClassification(
    ...
    for (const groupId of groupIds) {
        ...
        const rootCode = root.code === TREASURY_ROOT_GROUP_CODE
        ...
        const scopedCustomerId = path.find((node) => Boolean(node.customerId))?.customerId ?? null;
...
export async function ensureSystemRootGroups(tx: Transaction): Promise<{
    ...
    const treasuryGroupId = roots.find((group) => group.code === TREASURY_ROOT_GROUP_CODE)?.id;
    const customersGroupId = roots.find((group) => group.code === CUSTOMERS_ROOT_GROUP_CODE)?.id;

    if (!treasuryGroupId || !customersGroupId) {
        throw new CounterpartyGroupRuleError("System root groups are not available");
    }
...
export async function readMembershipMap(
    ...
    const rows = await db
        ...
        .from(schema.counterpartyGroupMemberships)
        .where(inArray(schema.counterpartyGroupMemberships.counterpartyId, counterpartyIds));
