...
export function createQuoteHandlers(
  ...
  async function getQuoteDetails(
    ...
    const feeComponents = await feesService.getQuoteFeeComponents({
      quoteId: quote.id,
    });
  ...
  async function listQuotes(
    ...
    if (validated.status && validated.status.length > 0) {
      conditions.push(inArray(schema.fxQuotes.status, validated.status as any));
    }
