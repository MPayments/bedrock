...
export class RequestTreasuryReturnCommand {
  constructor(
  ...
  async execute(
    raw: RequestTreasuryReturnInput,
  ): Promise<TreasuryInstruction> {
    const validated = RequestTreasuryReturnInputSchema.parse(raw);
    ...
    if (instruction.state === "return_requested") {
    ...
    const updated = await this.instructionsRepository.updateInstruction({
      ...
      returnRequestedAt: this.runtime.now(),
