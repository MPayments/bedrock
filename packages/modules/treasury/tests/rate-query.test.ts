...
import { GetCrossRateQuery } from "../src/rates/application/queries/get-cross-rate";
import { GetLatestRateQuery } from "../src/rates/application/queries/get-latest-rate";
...
function createHarness() {
  ...
  const deps = {
    ...
    getLatestManualRate: vi.fn(async (baseId: string, quoteId: string) =>
      manualRates.get(pairKey(baseId, quoteId)),
    ),
    getLatestRateBySource: vi.fn(
      async (
        baseId: string,
        quoteId: string,
        _asOf: Date,
  ...
  const getLatestRate = new GetLatestRateQuery(
    ...
    {
      ensureSourceFresh: deps.ensureSourceFresh,
      getLatestManualRate: deps.getLatestManualRate,
      getLatestRateBySource: deps.getLatestRateBySource,
    } as any,
  );
...
describe("rate query handlers", () => {
  it("returns manual latest rate without source refresh", async () => {
    ...
    const rate = await h.handlers.getLatestRate(
      "usd",
      "eur",
  ...
  it("returns the explicitly requested source without consulting manual rates", async () => {
    const h = createHarness();
    h.setManualRate("USD", "RUB", {
      source: "manual",
      rateNum: 99n,
      rateDen: 1n,
      asOf: new Date("2026-02-19T00:00:00.000Z"),
    ...
    expect(h.deps.getLatestManualRate).not.toHaveBeenCalled();
    expect(h.deps.ensureSourceFresh).toHaveBeenCalledTimes(1);
    expect(h.deps.ensureSourceFresh).toHaveBeenCalledWith(
      "investing",
      expect.any(Date),
    );
  });
  ...
  it("falls back to investing when cbr has no pair", async () => {
    ...
    expect(rate.source).toBe("investing");
    expect(rate.rateNum).toBe(91n);
    expect(h.deps.ensureSourceFresh).toHaveBeenNthCalledWith(
      1,
      "cbr",
      expect.any(Date),
    );
  ...
  it("throws when an explicitly requested source has no pair", async () => {
    const h = createHarness();

    await expect(
      h.handlers.getLatestRate(
        "USD",
        "RUB",
        new Date("2026-02-19T12:00:00.000Z"),
        "investing",
      ),
    ).rejects.toThrow(RateNotFoundError);
  ...
  it("returns identity rate for same currency", async () => {
    ...
    const rate = await h.handlers.getCrossRate(
      "USD",
      "USD",
      new Date("2026-02-19T12:00:00.000Z"),
    );
