...
import type { AccountingReportsDocumentsPort } from "../../../application/ports";
import type {
...
export function createReportsScopeHelpers(input: {
  ...
  async function resolveScope(inputArgs: {
    ...
    const invalidBookIds = bookRows
      .filter(
        (row) => !row.ownerId || !internalOrganizationIdSet.has(row.ownerId),
      )
      .map((row) => row.id);
  ...
  function buildScopeMeta(inputArgs: {
    ...
    return {
      ...
      resolvedCounterpartyIdsCount:
        inputArgs.scope.resolvedCounterpartyIds.length,
