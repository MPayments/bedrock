...
export function validatePostingMatrix(
  ...
  const activeAccounts = new Map(
    input.accounts.map((account) => [account.accountNo, account]),
  );
  const requiredAccountDimsByNo = new Map<string, Set<string>>();

  for (const row of input.accountDimPolicies) {
  ...
  for (const row of input.postingCodeDimPolicies) {
    if (!row.required) {
      continue;
    }
  ...
  for (const rule of input.rules.filter((row) => row.enabled ?? true)) {
    const key = [
      rule.postingCode,
      rule.debitAccountNo,
