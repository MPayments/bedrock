...
interface ValidateDealRouteInput {
...
export function validateDealRouteDefinition(
  input: ValidateDealRouteInput,
): DealRouteValidationIssue[] {
  const issues: DealRouteValidationIssue[] = [];
  const participantCodes = new Map<string, DealRouteParticipantInput>();
  const legCodes = new Map<string, DealRouteLegInput>();

  for (const participant of input.participants) {
    if (participantCodes.has(participant.code)) {
      pushIssue(issues, {
        code: "participant.code_duplicate",
  ...
  for (const [index, participant] of input.participants.entries()) {
    if (participant.sequence !== index + 1) {
      pushIssue(issues, {
        code: "participant.sequence_not_contiguous",
  ...
  for (const [index, leg] of input.legs.entries()) {
    ...
    if (!participantCodes.has(leg.fromParticipantCode)) {
      pushIssue(issues, {
        code: "leg.from_participant_missing",
    ...
    if (!participantCodes.has(leg.toParticipantCode)) {
      pushIssue(issues, {
        code: "leg.to_participant_missing",
    ...
    if (
      ...
      pushIssue(issues, {
        code: "leg.non_fx_currency_mismatch",
  ...
  if (input.legs.length > 0 && input.participants.length > 0) {
    ...
    if (visited.size !== input.participants.length) {
      pushIssue(issues, {
        code: "route.graph_disconnected",
        message: "Route participant graph must be connected",
        path: null,
      });
    }
  }
  ...
  for (const component of input.costComponents) {
    ...
    if (component.formulaType === "fixed" && !component.fixedAmountMinor) {
      pushIssue(issues, {
        code: "component.fixed_amount_missing",
    ...
    if (component.formulaType === "per_million" && !component.perMillion) {
      pushIssue(issues, {
        code: "component.per_million_missing",
    ...
    if (component.formulaType === "manual" && !component.manualAmountMinor) {
      pushIssue(issues, {
        code: "component.manual_amount_missing",
  ...
  if (!hasCustomer) {
    pushIssue(issues, {
      code: "route.customer_missing",
      message: "Route must include a customer participant",
      path: null,
    });
  }
  ...
  if (input.dealType === "payment" && !hasPayout) {
    pushIssue(issues, {
      ...
      message: "Payment routes must include a payout leg",
      path: null,
    });
  }
  ...
  if (input.dealType === "currency_exchange" && !hasFx) {
    pushIssue(issues, {
      code: "route.fx_leg_missing",
      message: "Currency exchange routes must include an FX conversion leg",
      path: null,
    });
  }
  ...
  if (
    ...
    pushIssue(issues, {
      code: "route.transit_flow_incomplete",
      message: "Transit-like routes must include both collection and payout legs",
      path: null,
    });
  }
