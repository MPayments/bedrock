...
import type {
  Database,
  Transaction,
} from "@bedrock/platform/persistence/drizzle";
import type {
  BedrockWorker,
  WorkerRunContext,
  WorkerRunResult,
...
async function listOperationBookIds(
  ...
  const result = await db.execute(sql`
    ...
    WHERE operation_id IN (${sql.join(
      operationIds.map((id) => sql`${id}::uuid`),
      sql`, `,
    )})
  `);
...
export function createLedgerWorkerDefinition(
  deps: {
    id?: string;
    intervalMs?: number;
    db: Database;
  ...
  async function postOperation(operationId: string) {
    ...
    const createPlans = plans.filter(
      (plan) =>
        plan.type === OPERATION_TRANSFER_TYPE.CREATE &&
        plan.status !== "posted",
    );
