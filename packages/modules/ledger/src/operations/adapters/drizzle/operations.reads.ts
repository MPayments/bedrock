...
import {
  ...
  type Dimensions,
  type LedgerOperationDetails,
  type LedgerOperationListRow,
  type ListLedgerOperationsInput,
} from "../../../contracts";
import { schema } from "../../../schema";
import type { LedgerOperationsReads } from "../../application/ports/operations.reads";
...
export class DrizzleOperationsReads implements LedgerOperationsReads {
  constructor(private readonly db: Queryable) {}

  list(
    input?: ListLedgerOperationsInput,
  ): Promise<PaginatedList<LedgerOperationListRow>> {
    return this.doList(input);
  }

  listDetails(
    operationIds: string[],
  ): Promise<Map<string, LedgerOperationDetails>> {
    return this.listOperationDetailsByIds(operationIds);
  }

  getDetails(operationId: string): Promise<LedgerOperationDetails | null> {
    return this.doGetDetails(operationId);
  }

  private async listOperationDetailsByIds(
    operationIds: string[],
  ): Promise<Map<string, LedgerOperationDetails>> {
    ...
    const postingsByOperationId = new Map<string, (typeof postingRows)[number][]>();
  ...
  private async doGetDetails(operationId: string) {
    return (await this.listOperationDetailsByIds([operationId])).get(operationId) ?? null;
  }
}
