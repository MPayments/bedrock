...
function normalizePartyKind(value: PartyKind): PartyKind {
  invariant(
    PARTY_KIND_VALUES.includes(value),
    `Unsupported counterparty kind: ${value}`,
    {
...
function normalizeRelationshipKind(
  ...
  invariant(
    COUNTERPARTY_RELATIONSHIP_KIND_VALUES.includes(value),
    `Unsupported counterparty relationship kind: ${value}`,
    {
