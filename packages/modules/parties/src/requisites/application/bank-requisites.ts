...
import {
  findRequisiteProviderIdentifier as findRequisiteProviderIdentifierLocal,
  formatRequisiteProviderAddress as formatRequisiteProviderAddressLocal,
  resolveRequisiteProviderDisplayName as resolveRequisiteProviderDisplayNameLocal,
} from "../../master-data";
import type { PartiesModule } from "../../module";
...
export function createCustomerBankingService(
  ...
  async function searchBankProviders(input: {
    ...
    for (const provider of providers) {
      ...
      const bic =
        findRequisiteProviderIdentifierLocal({
          provider,
          scheme: "bic",
        })?.value ?? null;
      const swift =
      ...
      merged.set(provider.id, {
        address: formatRequisiteProviderAddressLocal({ provider }),
        bic,
        ...
        displayLabel: buildProviderLabel({
          ...
          country: provider.country,
          name:
            resolveRequisiteProviderDisplayNameLocal({ provider }) ??
  ...
  async function upsertCounterpartyBankRequisite(input: {
    ...
    const bankValues = {
      ...
      label:
        ...
        resolveRequisiteProviderDisplayNameLocal({ provider: resolvedProvider }) ??
        normalizeNullableText(input.values.orgName) ??
        "Bank details",
      beneficiaryName,
