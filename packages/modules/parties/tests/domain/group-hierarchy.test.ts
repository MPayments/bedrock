...
} from "../../src/shared/domain/group-hierarchy";
...
describe("group hierarchy", () => {
  ...
  it("lists subtree ids and plans detachment", () => {
    const hierarchy = GroupHierarchy.create([...GROUPS]);

    expect(hierarchy.listSubtreeIds("shared-root")).toEqual([
      "shared-root",
      "shared-leaf",
    ]);
    expect(
      hierarchy.withoutCustomerScopedGroups([
        "customer-leaf",
        "shared-leaf",
        "shared-leaf",
      ]),
    ).toEqual(["shared-leaf"]);
    expect(hierarchy.wouldCreateCycle("customer-root", "customer-leaf")).toBe(
      true,
    );
    expect(hierarchy.wouldCreateCycle("shared-root", "customer-leaf")).toBe(
      false,
    );
    expect(
      hierarchy.planCustomerDetachment({
        customerId: "cust-abc",
        linkedCounterpartyIds: ["cp-1", "cp-1"],
        memberships: [
          {
            counterpartyId: "cp-1",
      ...
      linkedCounterpartyIds: ["cp-1"],
      removableGroupIds: ["customer-leaf"],
    });
  });
});
