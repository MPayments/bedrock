...
export interface IamIdentityStoreDeps {
  db: Database;
}

function buildUserListWhere(input: Parameters<IamIdentityStore["listUsers"]>[0]): SQL | undefined {
...
export function createDrizzleIamIdentityStore(
  deps: IamIdentityStoreDeps,
): IamIdentityStore {
  const { db } = deps;

  async function listUsers(
    input: Parameters<IamIdentityStore["listUsers"]>[0],
  ...
  async function findUserById(id: string): Promise<IamUserRecord | null> {
    const [row] = await db.select().from(user).where(eq(user.id, id)).limit(1);
    return row ?? null;
  }

  async function findUserByEmail(email: string): Promise<IamUserRecord | null> {
    const [row] = await db
      .select()
      .from(user)
  ...
  async function createUserWithCredential(
    input: Parameters<IamIdentityStore["createUserWithCredential"]>[0],
  ...
  async function updateUser(
    input: Parameters<IamIdentityStore["updateUser"]>[0],
  ...
  async function getUserWithLastSession(userId: string) {
    const [row] = await db
      .select()
      .from(user)
  ...
  async function banUser(
    input: Parameters<IamIdentityStore["banUser"]>[0],
  ): Promise<IamUserRecord | null> {
    return db.transaction(async (tx: Transaction) => {
  ...
  async function unbanUser(userId: string): Promise<IamUserRecord | null> {
    const [updated] = await db
      .update(user)
      .set({
        banned: false,
