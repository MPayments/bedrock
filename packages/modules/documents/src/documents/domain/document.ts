...
export class Document extends Entity<string> {
  ...
  static createDraft(input: CreateDocumentProps) {
    const summary = buildSummary(input.summary);

    return new Document({
      id: input.draft.id,
      docType: input.draft.docType,
      docNo: input.draft.docNo,
      moduleId: input.draft.moduleId,
      moduleVersion: input.draft.moduleVersion,
      payloadVersion: input.draft.payloadVersion,
  ...
  static fromSnapshot(snapshot: DocumentSnapshot) {
    return new Document(cloneSnapshot(snapshot));
  }
  ...
  updateDraft(input: UpdateDocumentDraftProps) {
    invariant(
  ...
  submit(input: {
    ...
    if (!canSubmit) {
      ...
      invariant(false, "Only draft documents can be submitted", {
  ...
  startPosting(input: {
    ...
    if (
      ...
      invariant(false, "Document is not ready for posting", {
