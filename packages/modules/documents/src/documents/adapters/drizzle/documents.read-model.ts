...
import type {
  DocumentsReadModel,
  FindDocumentIdByCreateIdempotencyKeyReadModelInput,
...
export class DrizzleDocumentsReadModel implements DocumentsReadModel {
  constructor(private readonly db: Queryable) {}

  async getDocumentByType(query: GetDocumentByTypeReadModelInput) {
  ...
  async findIncomingLinkedDocument(query: FindIncomingLinkedDocumentInput) {
  ...
  async findDocumentIdByCreateIdempotencyKey(
    query: FindDocumentIdByCreateIdempotencyKeyReadModelInput,
  ) {
    const [row] = await this.db
      .select({ id: schema.documents.id })
      .from(schema.documents)
  ...
  async listAdjustmentsForOrganizationPeriod(
    query: ListAdjustmentsForOrganizationPeriodInput,
  ) {
