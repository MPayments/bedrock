...
const idempotencyKeySchema = z
  ...
  .max(255);
const eventIdempotencyKeySchema = z
  .string()
  .min(1, "eventIdempotencyKey is required")
  .max(255);
const memoSchema = z
  .string()
  .max(1000, "memo cannot exceed 1000 characters")
  .optional();
...
export const TransferKindSchema = z.enum(["intra_org", "cross_org"]);

export const TransferSettlementModeSchema = z.enum(["immediate", "pending"]);
...
export type SettlePendingTransferInput = z.infer<
  typeof SettlePendingTransferInputSchema
>;

export const VoidPendingTransferInputSchema = settleVoidBaseSchema.extend({
  reason: reasonSchema.optional(),
});
export type VoidPendingTransferInput = z.infer<
  typeof VoidPendingTransferInputSchema
>;

const TRANSFERS_SORTABLE_COLUMNS = [
  "createdAt",
  "updatedAt",
  "approvedAt",
] as const;
...
interface TransfersListFilters {
  ...
  settlementMode: {
    kind: "string";
    cardinality: "multi";
    enumValues: readonly ["immediate", "pending"];
  };
  kind: {
    kind: "string";
    cardinality: "multi";
    enumValues: readonly ["intra_org", "cross_org"];
  };
}
...
export function validateVoidPendingTransferInput(
  input: unknown,
