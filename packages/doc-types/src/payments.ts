...
type PayoutResolvePayload = z.infer<typeof PayoutResolveSchema>;
type FeePayoutInitiatePayload = z.infer<typeof FeePayoutInitiateSchema>;
type FeePayoutResolvePayload = z.infer<typeof FeePayoutResolveSchema>;
...
async function requireParentCase(
  db: DocumentModuleContext["db"],
  documentId: string,
) {
  const [row] = await db
    ...
    .where(
      and(
        ...
        eq(schema.documentLinks.linkType, "parent"),
        eq(schema.documents.docType, "payment_case"),
      ),
    )
    .limit(1);
...
async function requireDependency(
  ...
  if (!row) {
    throw new DocumentValidationError(
      `depends_on link to ${expectedDocType} is required for ${documentId}`,
    );
  }
...
async function ensureOperationalAccountCurrency(
  db: DocumentModuleContext["db"],
  currenciesService: {
  ...
  if (!account) {
    throw new DocumentValidationError(
      `Operational account not found: ${operationalAccountId}`,
    );
  }
...
async function loadPendingTransferIdsForDocument(
  db: DocumentModuleContext["db"],
  documentId: string,
) {
...
async function ensureNoResolutionDocument(
  db: DocumentModuleContext["db"],
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(schema.documentLinks.toDocumentId, dependencyDocumentId),
        eq(schema.documentLinks.linkType, "depends_on"),
        inArray(schema.documents.docType, docTypes),
      ),
    );
  ...
  if (blocking) {
    throw new DocumentValidationError(
      `Document ${dependencyDocumentId} already has a resolution document`,
    );
  }
}

async function loadFxQuoteByRef(
  db: DocumentModuleContext["db"],
  currenciesService: {
    findById: (id: string) => Promise<{ code: string }>;
  },
  ...
  let quote:
    | (typeof schema.fxQuotes.$inferSelect & {
        fromCurrency?: string;
        toCurrency?: string;
      })
    | undefined;
...
async function consumeFxQuoteForExecution(
  db: DocumentModuleContext["db"],
  currenciesService: {
    findById: (id: string) => Promise<{ code: string }>;
  },
  input: FxExecutePayload,
) {
  const quote = await loadFxQuoteByRef(db, currenciesService, input.quoteRef);

  if (quote.fromCurrency !== input.payInCurrency) {
    throw new DocumentValidationError(
      `quote.fromCurrency mismatch: expected ${quote.fromCurrency}, got ${input.payInCurrency}`,
    );
  }
  if (quote.toCurrency !== input.payOutCurrency) {
    throw new DocumentValidationError(
      `quote.toCurrency mismatch: expected ${quote.toCurrency}, got ${input.payOutCurrency}`,
    );
  }
  if (quote.fromAmountMinor !== toMinor(input.principalMinor)) {
    throw new DocumentValidationError(
      `quote.fromAmountMinor mismatch: expected ${quote.fromAmountMinor.toString()}, got ${input.principalMinor}`,
    );
  }
  if (quote.toAmountMinor !== toMinor(input.payOutAmountMinor)) {
    throw new DocumentValidationError(
      `quote.toAmountMinor mismatch: expected ${quote.toAmountMinor.toString()}, got ${input.payOutAmountMinor}`,
    );
  }
  ...
  if (quote.status === "used") {
    ...
    throw new InvalidStateError(
      `Quote ${quote.id} is already used by ${quote.usedByRef ?? "unknown reference"}`,
    );
  }
...
async function loadFxRouteLegs(
  db: DocumentModuleContext["db"],
  currenciesService: {
    findById: (id: string) => Promise<{ code: string }>;
  },
...
async function buildPaymentCaseComputed(
  context: DocumentModuleContext,
  document: Document,
) {
  ...
  const dependencyRows =
    childIds.length > 0
      ? await context.db
          ...
          .where(
            and(
              inArray(schema.documentLinks.fromDocumentId, childIds),
              eq(schema.documentLinks.linkType, "depends_on"),
            ),
          )
      : [];

  const dependsOnByChild = new Map<string, string[]>();
...
export function createPaymentCaseDocumentModule(): DocumentModule<
  PaymentCasePayload,
  PaymentCasePayload
> {
  return {
  ...
  return {
    ...
    async canPost(context, document) {
      ...
      await Promise.all([
        ...
        ensureOperationalAccountCurrency(
          context.db,
...
function createPayoutResolveModule(params: {
  docType: "payout_settle" | "payout_void";
  docNoPrefix: string;
  eventType: "settle" | "void";
}) {
  return function createModule(): DocumentModule<
    PayoutResolvePayload,
    PayoutResolvePayload
  > {
    return {
      ...
      async canCreate(context, input) {
        await requireDocument(
          context.db,
          input.payoutInitiateDocumentId,
          "payout_initiate",
          "payout_initiate",
        );
      },
      ...
      async buildInitialLinks(context, document) {
        const payload = parsePayoutResolvePayload(document);
        const dependency = await requireDocument(
          context.db,
          payload.payoutInitiateDocumentId,
          "payout_initiate",
          "payout_initiate",
...
export function createPayoutInitiateDocumentModule(deps: {
  ...
  return {
    ...
    async canPost(context, document) {
      ...
      if (caseDocument.id !== payload.caseDocumentId) {
        throw new DocumentValidationError(
          "payment_case link does not match payout_initiate payload",
        );
      }
...
function createFeePayoutResolveModule(params: {
  docType: "fee_payout_settle" | "fee_payout_void";
  docNoPrefix: string;
  eventType: "settle" | "void";
}) {
  return function createModule(): DocumentModule<
    FeePayoutResolvePayload,
    FeePayoutResolvePayload
  > {
    return {
      docType: params.docType,
      ...
      async canCreate(context, input) {
        await requireDocument(
          context.db,
          input.feePayoutInitiateDocumentId,
          "fee_payout_initiate",
          "fee_payout_initiate",
        );
      },
      ...
      async canPost(context, document) {
        ...
        if (dependency.id !== payload.feePayoutInitiateDocumentId) {
          throw new DocumentValidationError(
            "feePayoutInitiateDocumentId does not match dependency",
          );
        }
...
export function createFeePayoutInitiateDocumentModule(deps: {
  ...
  return {
    ...
    async canPost(context, document) {
      ...
      if (caseDocument.id !== payload.caseDocumentId) {
        throw new DocumentValidationError(
          "payment_case link does not match fee_payout_initiate payload",
        );
      }
      ...
      const component = fxDetails.separateFeeComponents.find(
        (item) => item.componentId === payload.componentId,
      );
      if (!component) {
        throw new DocumentValidationError(
          `FX execution does not expose separate fee component ${payload.componentId}`,
        );
      }
      ...
      if (component.accountingTreatment !== payload.accountingTreatment) {
        throw new DocumentValidationError(
          "accountingTreatment does not match fx_execute component",
        );
      }
