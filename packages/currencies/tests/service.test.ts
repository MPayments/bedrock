...
describe("createCurrenciesService", () => {
    ...
    it("throws CurrencyNotFoundError for unknown code and id", async () => {
        ...
        await expect(service.findByCode("RUB")).rejects.toThrow(CurrencyNotFoundError);
        await expect(service.findById("missing-id")).rejects.toThrow(CurrencyNotFoundError);
    });
    ...
    it("invalidates cache after update and throws when update target missing", async () => {
        ...
        await expect(service.update("missing-id", { name: "Nope" })).rejects.toThrow(CurrencyNotFoundError);
    });
});
