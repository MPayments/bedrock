...
export async function tbCreateAccountsOrThrow(
  tb: TbClient,
  accounts: Account[],
) {
  ...
  const hard = errs
    ...
    .map((e) => ({
      index: e.index,
      code: e.result,
      name: enumName(CreateAccountError, e.result),
    }));
...
export async function tbCreateTransfersOrThrow(
  tb: TbClient,
  transfers: Transfer[],
) {
  ...
  const hard = errs
    ...
    .map((e) => ({
      index: e.index,
      code: e.result,
      name: enumName(CreateTransferError, e.result),
    }));
