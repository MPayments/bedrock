...
import {
  type CounterpartyCardParseResult,
  type CounterpartyInnLookupResult,
  lookupCounterpartyByInn,
  parseCounterpartyCardPdf,
} from "@/lib/counterparty-prefill";

export type CounterpartyPrefillPatch = {
  general: Partial<CounterpartyGeneralFormValues>;
  profile: Partial<PartyProfileBundleInput>;
  ignoredBanking?: boolean;
};

export type CounterpartyInputMethod = "manual" | "inn" | "card";

type CounterpartyInputMethodCardProps = {
  counterpartyKind: "legal_entity" | "individual";
  disabled?: boolean;
  mode: CounterpartyInputMethod;
  onModeChange: (mode: CounterpartyInputMethod) => void;
  onPrefill: (patch: CounterpartyPrefillPatch) => void;
};
...
type ModeOption = {
  ...
  ruOnly?: boolean;
  title: string;
  value: CounterpartyInputMethod;
};

export function CounterpartyInputMethodCard({
  counterpartyKind,
  disabled,
  mode,
  onModeChange,
  onPrefill,
}: CounterpartyInputMethodCardProps) {
  const isLegal = counterpartyKind === "legal_entity";
  ...
  return (
    <Card>
      <CardHeader className="border-b">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Выберите удобный путь — выбор не блокирует ручное редактирование полей ниже.
          </p>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {mode === "inn" && isLegal ? (
          <InnLookupPanel disabled={disabled} onPrefill={onPrefill} />
        ) : null}
        {mode === "card" && isLegal ? (
          <CardParsePanel disabled={disabled} onPrefill={onPrefill} />
        ) : null}
      </CardContent>
    </Card>
  );
...
function InnLookupPanel({ disabled, onPrefill }: PrefillPanelProps) {
  ...
  async function handleLookup() {
    ...
    try {
      const data = await lookupCounterpartyByInn(value);
  ...
  return (
    <div className="space-y-3 rounded-lg border bg-muted/20 p-4">
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          Найдём карточку в ЕГРЮЛ/ЕГРИП и автоматически заполним реквизиты, адрес и данные подписанта.
        </p>
      </div>
      ...
      {success ? (
        <div className="flex items-center gap-2 rounded-md bg-emerald-500/10 px-3 py-2 text-xs text-emerald-700 dark:text-emerald-400">
          ...
          Данные контрагента подгружены в форму ниже
        </div>
      ) : null}
      {error ? (
...
function CardParsePanel({ disabled, onPrefill }: PrefillPanelProps) {
  ...
  async function handleFile(file: File | undefined) {
    ...
    try {
      const data = await parseCounterpartyCardPdf(file);
        ...
        <div className="space-y-2 rounded-lg border bg-emerald-500/10 p-4">
          ...
          {ignoredBanking ? (
            <div className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
              В файле найдены банковские реквизиты. Они не переносятся в карточку контрагента — добавьте вручную при необходимости.
            </div>
          ) : null}
        </div>
      ) : (
...
function buildPatchFromInn(
  data: CounterpartyInnLookupResult,
): CounterpartyPrefillPatch {
  return {
...
function buildPatchFromCard(
  data: CounterpartyCardParseResult,
): CounterpartyPrefillPatch {
...
function buildGeneralPatch(
  data: CounterpartyInnLookupResult,
): Partial<CounterpartyGeneralFormValues> {
  const patch: Partial<CounterpartyGeneralFormValues> = {
    country: "RU",
  };
...
function buildProfilePatch(
  ...
  const pushIdentifier = (scheme: PartyIdentifierInput["scheme"], value: string | undefined) => {
    if (value) {
...
function buildContacts(data: CounterpartyCardParseResult): PartyContactInput[] {
...
function buildDirector(
  ...
  return {
    ...
    fullNameI18n: data.directorName ? { ru: data.directorName, en: null } : null,
