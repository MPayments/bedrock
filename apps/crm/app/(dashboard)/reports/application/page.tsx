...
export default function ApplicationReportsPage() {
  ...
  // Начальные фильтры: все статусы кроме rejected, hasCalculation = true
  const initialStatusFilter: Array<"forming" | "created" | "finished"> = [
    "forming",
    "created",
    "finished",
  ];
  ...
  // Убираем пагинацию - устанавливаем очень большой pageSize
  const {
    data,
    loading,
    error,
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      try {
        ...
        const url = `${API_BASE_URL}/applications/by-day?${params.toString()}`;
        ...
        // When reportCurrencyCode is provided, the API returns { data, reportCurrencyCode }
        const rawData: ChartDataPoint[] = Array.isArray(json)
          ? json
          : json.data ?? [];

        // Нормализуем данные: для каждой валюты всегда есть число (включая 0),
        // чтобы линии и area-график не «обрывались» при отсутствии сделок в валюте в конкретный день.
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const url = `${API_BASE_URL}/applications/export-excel?${params.toString()}`;
      const res = await fetch(url, {
        credentials: "include",
      });
      ...
      let filename = `applications-report-${
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold">Отчёты по заявкам</h1>
        </div>
        <div className="flex items-center gap-4">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex flex-col gap-2 pt-8 text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="whitespace-nowrap">Всего на дату</span>
                  </div>
                </div>
