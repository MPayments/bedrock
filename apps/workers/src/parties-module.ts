...
export function createWorkerPartiesModule(input: {
  db: Database;
  logger: Logger;
  ...
  return createPartiesModule({
    ...
    legalEntityReads: new DrizzleLegalEntitiesReads(input.db),
...
export function createWorkerPartiesReadRuntime(database: Database | Transaction) {
  return {
