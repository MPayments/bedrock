...
async function buildDealDocumentPrintContext(input: {
  ...
  const fallbackAmount =
    document?.amountMinor != null && fallbackCurrency
      ...
      : readPayloadString(document?.payload, "amount") ?? deal.amount;
  ...
  return {
    ...
    deal: {
      ...
      applicationNumber: document?.docNo ?? null,
...
export async function generateDocumentPrintForm(input: {
  ...
  return input.ctx.documentGenerationWorkflow.generateDealDocument({
    ...
    templateType: definition.templateType as "acceptance" | "invoice",
  });
}
...
export async function listDealPrintForms(input: {
  ...
  return descriptorsFor({
    ownerType: "deal",
    warnings: context.warnings.filter(
