...
const PaginatedCounterpartiesSchema = createPaginatedListSchema(CounterpartySchema);
    ...
    return app
        ...
        .openapi(updateRoute, async (c) => {
            ...
            try {
                ...
                if (err instanceof CounterpartyNotFoundError || err instanceof CounterpartyGroupNotFoundError || err instanceof CounterpartyCustomerNotFoundError) {
                    return c.json({ error: err.message }, 404);
                }
                if (err instanceof CounterpartyGroupRuleError) {
                    return c.json({ error: err.message }, 400);
