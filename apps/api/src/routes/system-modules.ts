...
import type { ModuleCatalogEntry } from "@bedrock/module-runtime";
...
export function systemModulesRoutes(ctx: AppContext) {
  ...
  app.get("/", requirePermission({ system_modules: ["list"] }), async (c) => {
  ...
  app.get(
    ...
    async (c) => {
      ...
      const bookId = c.req.query("bookId") || undefined;
      const effective = await ctx.moduleRuntime.getEffectiveState({
        moduleId,
        bookId,
      });
