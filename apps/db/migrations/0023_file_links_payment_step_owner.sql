-- Second half of the payment-step owner extension for `file_links`.
-- The preceding 0022 migration committed the `payment_step_evidence` enum
-- value so it may now be referenced in the updated CHECK constraints.
...
ALTER TABLE "file_links" ADD CONSTRAINT "file_links_generated_variant_shape_chk" CHECK ((
        "file_links"."link_kind"::text in ('deal_attachment', 'legal_entity_attachment', 'payment_step_evidence')
