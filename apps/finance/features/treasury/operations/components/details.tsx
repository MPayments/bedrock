...
function InstructionActionsAdminNotice({
  operation,
}: {
  operation: TreasuryOperationDetails;
}) {
  return (
    <Card className="rounded-sm">
      <CardHeader className="border-b">
        ...
        <CardDescription>
          Движение инструкции перенесено в карточку шага на рабочем столе
          сделки. Этот экран теперь только для аудита.
        </CardDescription>
      </CardHeader>
      <CardContent className="pt-4 text-sm text-muted-foreground">
        {operation.dealWorkbenchHref ? (
          ...
          >
            Перейти к карточке шага
          </Button>
        ) : (
          <span>
            Операция не привязана к сделке — управляйте через admin-API.
          </span>
        )}
      </CardContent>
    </Card>
    ...
    >
      <div className="grid gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
        <div className="grid gap-4">
          <InstructionActionsAdminNotice operation={operation} />
