...
const segmentMap: Record<string, SegmentConfig> = {
  ...
  operations: { label: "Операции", href: "/treasury/operations", icon: "workflow" },
  journal: { label: "Журнал операций" },
  compose: { label: "Маршрут" },
  calculation: { label: "Расчет" },
  execution: { label: "Исполнение" },
  reconciliation: { label: "Сверка" },
  "route-templates": {
