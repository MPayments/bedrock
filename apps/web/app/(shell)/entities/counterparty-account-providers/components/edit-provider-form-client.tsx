...
export function EditProviderFormClient({ provider }: EditProviderFormClientProps) {
  ...
  async function handleSubmit(
    ...
    const result = await executeMutation<ProviderDetails>({
      request: () =>
        apiClient.v1["counterparty-account-providers"][":id"].$patch({
  ...
  async function handleDelete(): Promise<boolean> {
    ...
    const result = await executeMutation<void>({
      request: () =>
        apiClient.v1["counterparty-account-providers"][":id"].$delete({
          param: { id: provider.id },
        }),
    ...
    router.push("/entities/counterparty-account-providers");
    return true;
  }
