...
import type { AccountsListResult } from "./types";
...
function createAccountsListQuery(search: AccountsSearchParams) {
  return createResourceListQuery(COUNTERPARTY_ACCOUNTS_LIST_CONTRACT, search);
}
...
const getAccountByIdUncached = async (
  ...
  return readEntityById({
    ...
    request: async (validId) => {
      const client = await getServerApiClient();
      return client.v1["counterparty-accounts"][":id"].$get(
        { param: { id: validId } },
        { init: { cache: "no-store" } },
      );
    },
