...
type EditCounterpartyRequisiteFormClientProps = {
  requisite: CounterpartyRequisiteDetails;
  options: CounterpartyRequisiteFormOptions;
};

export function EditCounterpartyRequisiteFormClient({
  requisite,
  options,
}: EditCounterpartyRequisiteFormClientProps) {
  ...
  async function handleSubmit(
    ...
    const result = await executeMutation<CounterpartyRequisiteDetails>({
      request: () =>
        apiClient.v1["counterparty-requisites"][":id"].$patch({
          param: { id: current.id },
          json: {
      ...
      fallbackMessage: "Не удалось обновить реквизит контрагента",
      parseData: async (response) => {
        ...
        return {
          id: payload.id,
          ownerId: payload.counterpartyId,
    ...
    toast.success("Реквизит контрагента обновлён");
    router.refresh();
    return result.data;
  }
  ...
  async function handleDelete() {
    ...
    const result = await executeMutation<void>({
      request: () =>
        apiClient.v1["counterparty-requisites"][":id"].$delete({
          param: { id: current.id },
        }),
    ...
    router.push("/entities/counterparty-requisites");
    return true;
  }
  ...
  return (
    <RequisiteGeneralForm
      ...
      ownerDescription="Внешний владелец реквизита."
